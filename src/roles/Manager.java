...
public class Manager {
  ...
  private static void findTrips(Scanner keyboard) throws Exception {
    ...
    try {
      ...
      rs = stmt.executeQuery("SELECT TEMP.TID, D.Dname, P.Pname, TEMP.Start_location, TEMP.Destination, TEMP.Duration FROM (SELECT T.TID, T.Start_location, T.Destination, T.Fee AS Duration, T.DID, T.PID, ABS(TS2.Location_x-TS1.Location_x)+ABS(TS2.Location_y-TS1.Location_y) AS Distance FROM Trips T, Taxi_stops TS1, Taxi_stops TS2 WHERE T.Start_location = TS1.Tname AND T.Destination = TS2.Tname) AS TEMP, Drivers D, Passengers P WHERE TEMP.PID = P.PID AND TEMP.DID = D.DID AND Distance >= "+min+" AND Distance <= "+max+";");
